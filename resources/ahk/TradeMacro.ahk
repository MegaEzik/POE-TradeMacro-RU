...
TradeFunc_OpenWikiHotkey(priceCheckTest = false, itemData = "") {
	...
	Else {
		...
		If (TradeOpts.WikiAlternative) {
			...
			} Else If (RegExMatch(Item.Name_En, "i)Sacrifice At|Fragment of|Mortal |Offering to |'s Key|Breachstone|Simulacrum")) {
				;UrlAffix := Item.Name
				UrlAffix := Item.Name_En
			} Else {
...
TradeFunc_Main(openSearchInBrowser = false, isAdvancedPriceCheck = false, isAdvancedPriceCheckRedirect = false, isItemAgeRequest = false)
{
			...
			Else If (Item.IsSynthesisedBase) {
				...
				For key, val in preparedItem.mods {
					If (val.type = "implicit") {
						...
						Item.Implicit.name_orig.push(val)
					} 
				}				
	...
	*/
	...
	If ((((RegExMatch(RequestParams.name, "[а-яА-ЯёЁ]+") or RegExMatch(RequestParams.xbase, "[а-яА-ЯёЁ]+") or not (StrLen(RequestParams.name) or StrLen(RequestParams.xtype) or StrLen(RequestParams.xbase))) and not isAdvancedPriceCheck) or (isAdvancedPriceCheck and Item.IsUnique and not uniqueWithVariableMods)) and not TradeOpts.UsePredictedItemPricing) {
...
TradeFunc_ParseRequestErrors(response) {
	...
	If (httpCode1 = "000") {
		...
		error := "ОШИБКА: Клиент был отключен до завершения запроса к poe.trade."
		error .= "`n`n" "Возможно требуется увеличить время запроса (Текущее значение " TradeOpts.CurlTimeout "с)."
		error .= "`n" "Вы можете сделать это в меню настроек на вкладке ""TradeMacro""."
		error .=  "`n`n" "Это может быть временной проблемой, из-за медленных ответов от сервера."
	}
...
;console_log(n, "N")			
			...
			If (not negativeToPositiveRange) {
				...
				t2 := RegExReplace(t, "i)(уменьшение)", "(увеличение)")
		...
		If (modFound) {
			...
			If (negativeToPositiveRange and negativeValue) {
				...
				v.name := RegExReplace(v.name, "i)(.*#%?) увеличение", "$1 уменьшение")
				v.name_orig := RegExReplace(v.name_orig, "i)(.*-?[\d\.]+|-?\(.+?\))(%?) увеличение", "$1$2$3 уменьшение")
				v.param := RegExReplace(v.param, "i)(.*#%?) увеличение", "$1 уменьшение")
...
TradeFunc_GetEnchantment(_item, type) {
	...
	For key, val in _item[searchKey] {
		...
		RegExMatch(_item[searchKey][key], "i)([.0-9]+)(%? до([.0-9]+))?", values)
		imp      := RegExReplace(_item[searchKey][key], "i)([.0-9]+)", "#")
		imp:=AdpRu_ConvertRuOneModToEn(imp)
...
TradeFunc_GetModValueGivenPoeTradeMod(itemModifiers, poeTradeMod, mods_const_ru) {
	...
	{		
		...
		_LoopField_ := mods_const_ru[LoopField]
		_LoopField_ := _LoopField_ ? _LoopField_ : LoopField
		CurrValue := TradeFunc_GetActualValue(_LoopField_, poeTradeMod_ValueTypes)
		...
		If (CurrValue ~= "\d+") {
			...
			If (values3) {
				...
				ModStr := StrReplace(LoopField, CurrValue, "# до #")
			}
			; handle single value
			Else {
...
TradeFunc_AdvancedPriceCheckGui(advItem, Stats, Sockets, Links, UniqueStats = "", ChangedImplicit = "") {
	...
	If (advItem.specialBase or advItem.IsBeast) {
		...
		advItem.iLvl 											; use itemlevel to fill the box in any case (elder/shaper/crusader/redeemer/hunter/warlord
	}
	...
	If (advItem.IsBeast) {
		...
		Gui, SelectModsGui:Add, CheckBox, x+15 yp+3 vTradeAdvancedSelectedItemBase %baseCheckState%, % "База предмета"
	}
...
TradeFunc_AprilFools() {
	...
	text := "Не удалось получить доступ к фрагменту памяти!`n`n"
	text .= "Ошибка Каваса 000000 в "
	text .= """C:\Program Files\Synthesis\`n3.6\modules\april\fools\bin\readFragment.exe"""
	;MsgBox, 0x12, Error, %text%, 
	MsgBox, 0x12, Ошибка, %text%, 
	IfMsgBox, Retry 
	{
		TradeFunc_AprilFools()
	}
