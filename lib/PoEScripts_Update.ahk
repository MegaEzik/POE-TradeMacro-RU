...
GetLatestRelease(user, repo, ReleaseVersion, ShowUpdateNotification, userDirectory, isDevVersion, skipSelection, skipBackup, SplashScreenTitle = "", debug = 0, repeatedCheck = false) {
	...
	Try  {
		...
		If (newRelease and (repo = "PoE-TradeMacro" || repo = "PoE-TradeMacro_ru")) {
			Menu, Tray, Icon, %A_ScriptDir%\resources\images\poe-trade-bl-update.ico
		}
		
		;Загрузка истории изменений для русской версии
		If (newRelease and repo = "PoE-TradeMacro_ru") {
			ruUpdNotesUrl:="https://raw.githubusercontent.com/" user "/" repo "/master/resources/UpdatesRu.txt"
			UrlDownloadToFile, %ruUpdNotesUrl%, temp\ruUpdNotes.txt
			sleep 25
			FileRead, updateNotes, temp\ruUpdNotes.txt
		}
		
		If (newRelease and repeatedCheck) {
			;TrayTip, %repo%, Update available.
			TrayTip, %repo%, Доступно обновление.
		}
		Else If (newRelease) {
			...
			Gui, UpdateNotification:Add, GroupBox, w630 h80 cGreen, Доступно обновление!	
			If (isPrerelease) {
			...
			Gui, UpdateNotification:Add, Text, x10 cGreen, Примечание к обновлению:
			...
			Gui, UpdateNotification:Add, Button, gCloseUpdateWindow, Закрыть
			...
			Gui, UpdateNotification:Show, w650 xCenter yCenter, Обновление
			;ControlFocus, Update, Update
			ControlFocus, Обновить, Обновление
			;WinWaitClose, Update
			WinWaitClose, Обновление
...
DownloadRelease(url, project, ByRef savePath) {	
	...
	SplashTextOn, 300, 20, Обновление %project%, Загрузка .zip архива...
...
ExtractRelease(file, project) {
	...
	SplashTextOn, 300, 20, Обновление %project%, Распаковка загруженного .zip архива...
